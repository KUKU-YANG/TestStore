					...
					<table class="table table-bordered">
						<tbody>
							...
							<tr class="active">
								...
								<td>
									<a href="javascript:;" id="${item.product.pid }" class="delete">移除</a>
								</td>
							</tr>
							</c:forEach>
						</tbody>
...
<script type="text/javascript">
	$(function() {
		//页面加载完之后获取class为delete的元素，为其绑定点击事件
		$(".delete").click(function() {
			if(confirm("是否要移除该商品?")){
				//获取被移除商品的pid
				var pid=this.id;
				window.location.href="/store01/CartServlet?method=removeCartItem&pid="+pid;
			}
		});
		$(".clear").click(function() {
			if(confirm("是否要清空购物车?")){
				window.location.href="/store01/CartServlet?method=clearCartItem";
			}
		});
